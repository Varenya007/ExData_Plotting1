...
volt<-as.numeric(new_dat$Voltage)
activepwr<- as.numeric(new_dat$Global_active_power)
reactivepwr<- as.numeric(new_dat$Global_reactive_power)
...
#using matrix to position the plots
par(mfrow=c(2,2)) # 2 plots in 1 row and 2 in 1 col

#base plotting
plot(date_time,activepwr, type="l", xlab="", ylab="Global Active Power")
...
#creating plot for reactive power
plot(date_time,reactivepwr, type="l", xlab="", ylab="Global Reactive Power")
#closing devices
dev.off()